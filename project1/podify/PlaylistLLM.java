...
// -------------------------------------------------------------------------
/**
 * Reads and writes the CSV song list made by an LLM, and holds the valid songs.
 * The columns are id,title,artist,duration,genre,playCount. Broken rows
 * (invalid format) are skipped instead of crashing the program.
 *
...
public class PlaylistLLM
{
    ...
    // ----------------------------------------------------------
    /**
     * Writes songs in the same format loadSongs reads, so the playlist and its
     * play counts survive closing the program.
     *
     * @param fileName
    ...
    // ----------------------------------------------------------
    /**
     * Saves the user's own playlist, whose play counts change as the program
     * runs.
     *
     * @param fileName
     *            the path to write to
     ...
     * @param usedIds
     *            the ids already seen in this file
     * @param loaded
     *            the list the new Song is added to
     */
    private void readRow(String line, List<String> usedIds, List<Song> loaded)
    {
    ...
    private void buildSkippedMessage(int loadedCount)
    {
        ...
        if (damagedRows > 0)
        {
            message = message + ", skipped " + damagedRows + " damaged row(s)";
        }
     ...
     *            the song to write
     * @param position
     *            where the song sits in the list
    ...
    // ----------------------------------------------------------
    /**
     * Checks whether any field holds a comma, which a CSV file cannot store.
     *
     * @param song
     *            the song to check
    ...
    // ----------------------------------------------------------
    /**
     * Checks that a duration looks like M:SS and holds real seconds, so "3:75"
     * is refused.
     *
     * @param text
     *            the duration from the file
